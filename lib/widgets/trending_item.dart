...
import 'package:flutter_shopping_ui_kit/widgets/clip_shadow_path.dart';
...
class TrendingItem extends StatelessWidget {
    ...
    return GestureDetector(
      child: Stack(
        children: <Widget>[
          ...
          Positioned(bottom: 0, left: trendCardWidth / 2 - rectWidth / 2, child: _cartIcon(rectWidth, rectHeight)),
        ],
      ),
      onTap: () {
      ...
      children: <Widget>[
        ...
        Center(
          child: Container(
            width: 100,
            height: 80,
            decoration: BoxDecoration(image: DecorationImage(image: AssetImage(product!.icon!), fit: BoxFit.contain)),
          ),
        )
      ],
    );
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        Row(
          children: <Widget>[
            Text(product!.price!,
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Color(0XFF85c2ff))),
            const Text(
              '#00.000',
  ...
  _cartIcon(double rectWidth, double rectHeight) {
    return ClipShadowPath(
      shadow: const Shadow(color: Colors.blue, blurRadius: 5),
...
class ProductImageContainerClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.cubicTo(0, size.height - 20, 0, size.height, size.width - 20, size.height - 6);
